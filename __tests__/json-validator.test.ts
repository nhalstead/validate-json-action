...
describe('Json validation results', () => {
    test('all successful when all jsons in the list are valid', async () => {
        ...
        expect(results.every(r => r.filePath.replace(/\\{1,2}/g, '/') === `${mocks_dir}/${validDataFile}`)).toBeTruthy();
    });
    ...
    test('only one failure when one json in the list is invalid', async () => {
        ...
        expect(successes.every(r => r.filePath.replace(/\\{1,2}/g, '/') === `${mocks_dir}/${validDataFile}`)).toBeTruthy();

        const failures = results.filter(r => !r.valid);
        expect(failures.length).toEqual(1);
        expect(failures.every(r => r.filePath.replace(/\\{1,2}/g, '/') === `${mocks_dir}/${invalidDataFile}`)).toBeTruthy();
    });
    ...
    test('all failures when all jsons in the list are invalid', async () => {
        ...
        expect(results.every(r => !r.valid)).toBeTruthy();
        expect(results.every(r => r.filePath.replace(/\\{1,2}/g, '/') === `${mocks_dir}/${invalidDataFile}`)).toBeTruthy();
    });
    ...
    test('one failures when schema file are invalid', async () => {
        ...
        expect(results.every(r => !r.valid)).toBeTruthy();
        expect(results.every(r => r.filePath.replace(/\\{1,2}/g, '/') === `${mocks_dir}/${invalidSchemaFile}`)).toBeTruthy();
    });
    ...
    test('one failure when no schema file', async () => {
        ...
        expect(results).toHaveLength(1);
        expect(results.every(r => !r.valid)).toBeTruthy();
        expect(results.every(r => r.filePath.replace(/\\{1,2}/g, '/') === mocks_dir)).toBeTruthy();
    });
